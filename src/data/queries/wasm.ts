...
/* token balance */
const useGetTokenBalanceQuery = (address: TerraAddress) => {
  const lcd = useLCDClient();
...
export const useTokenBalance = (token: AccAddress, address: TerraAddress) => {
  const getQuery = useGetTokenBalanceQuery(address);
...
export const useCW721Tokens = (contract: AccAddress, address: TerraAddress) => {
  const getQuery = useGetContractQuery();
